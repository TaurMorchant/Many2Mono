...
| `BRANCHES` | Список веток для клонирования и объединения (через пробел) | `main` |
...
### Шаг 1: clone
1. Для каждой ветки из `BRANCHES`:
   - Проверяет существование ветки в удалённом репозитории
   - Если ветка отсутствует — **прерывает выполнение с ошибкой**
   - Клонирует репозиторий как bare в `tmp/<repo>-<branch>.git`
   - Применяет `git filter-repo --to-subdirectory-filter` для переноса файлов в субдиректорию

### Шаг 2: merge
1. Для каждой ветки из `BRANCHES`:
   - Первая ветка: создаёт пустую монорепу в `monorepo/`
   - Остальные ветки: создаются как orphan branches (независимые истории)
   - Мёржит все уже склонированные репозитории из `tmp/` с `--allow-unrelated-histories`
...
Для переноса LTS веток из исходных репозиториев в монорепу:

1. **Настройка в config.env:**
   ```bash
...
3. **Результат:**
   - Создаётся монорепа с несколькими ветками
   - Каждая ветка содержит все модули в соответствующем состоянии
   - Ветки являются независимыми историями (orphan branches)
   - `git log --all --graph` показывает отдельные деревья для каждой ветки

4. **Проверка:**
   ```bash
   cd monorepo
   git branch -a          # Список всех веток
   git checkout lts/25.3  # Переключение на LTS ветку
   ```

**Важно:** Все репозитории должны иметь все указанные в `BRANCHES` ветки. Если хотя бы одна ветка отсутствует в каком-либо репозитории, процесс прервётся с ошибкой.
